...
def scrape_query(page, query: str, category: str, seen_urls: set, today_only: bool = True) -> list[dict]:
    ...
    for page_num in range(1, MAX_PAGES + 1):
        location_param = JOB_LOCATION.lower().replace(" ", "+")
        url = (
            f"{BASE_URL}/jobs?q={query.replace(' ', '+')}&location={location_param}"
            if page_num == 1
            else f"{BASE_URL}/jobs?q={query.replace(' ', '+')}&location={location_param}&page={page_num}"
        )
